...
class RRT_arm():
    ...
    def __init__(self, lower_arm_length, upper_arm_length, height_base, dt=0.01):
    ...
    def inverse_kinematics(self, desired_position):
        ...
        J11 = -1 * np.sin(self.theta) * (
                self.lower_arm_length * np.cos(self.phi) + self.upper_arm_length * np.cos(self.phi + self.psi))
        J21 = np.cos(self.theta) * (
                self.lower_arm_length * np.cos(self.phi) + self.upper_arm_length * np.cos(self.phi + self.psi))
        J31 = 0
    ...
    def draw_arm(self):
        ...
        x2 = np.cos(self.theta) * (
                self.lower_arm_length * np.cos(self.phi) + self.upper_arm_length * np.cos(self.phi + self.psi))
        y2 = np.sin(self.theta) * (
                self.lower_arm_length * np.cos(self.phi) + self.upper_arm_length * np.cos(self.phi + self.psi))
        z2 = (self.height_base + self.lower_arm_length * np.sin(self.phi) + self.upper_arm_length * np.sin(
            self.phi + self.psi))
    ...
    def RRT(self):
        ...
        for j in range(self.point_list.shape[0]):
            ...
            evaluation_value = np.sqrt(
                (difference_values[0] ** 2) + (difference_values[1] ** 2) + (difference_values[2] ** 2))
    ...
    def collides_with_obstacle(self, begin_point, end_point):
        ...
        while i < len(limits):
            ...
            for j in range(50):
                ...
                # collision checks both lower and upper limit of each obstacle
                collision = x >= limits[i][0] - safety_factor and x <= limits[i + 1][0] + safety_factor \
        ...
        for i in tqdm(range(steps)):
            ...
            for j in range(steps):
                ...
                for k in range(steps):
                    ...
                    if self.check_collision(theta_pos, phi_pos, psi_pos):
                        dataset[i, j, k] = True
    ...
    def check_collision(self, theta_pos, psi_pos, phi_pos):
        ...
        if (arm_config[:, 2] < -0.01).any(): # Check if the arm collides with the floor
            ...
            return True
        elif self.collides_with_obstacle(arm_config[1], arm_config[2]) == False:  # Lower arm obstacle_detection
            return True
        elif not self.collides_with_obstacle(arm_config[2], arm_config[3]):  # Upper arm obstacle_detection
            return True
        else:
            return False
    ...
    def find_path(self):
        ...
        path = np.array([self.goal])
        index = goal_index[np.where(cost_list == min(cost_list))[0][0]]
        while (path[-1, :] != self.begin_configuration).all():
            last_point = self.line_list[index, 0, :]
            path = np.append(path_list_temp, np.array([last_point]), axis=0)
            for j in range(self.line_list.shape[0]):
                if ((self.line_list[j, 1, :] == self.line_list[index, 0, :]).all()):
                    index = j
    ...
    def RRT_plot(self, amount_nodes, amount_obstacles, animate_plot=True, save_animation=False, method='RRT'):
        ...
        ax_rrt.voxels(self.x_obs, self.y_obs, self.z_obs, self.data_obstacles, facecolors=[1, 0, 0, 0.1],
                      edgecolors='grey')
        ...
        def generate_configurations(n):
            for i in range(n):
                if method == 'RRT':
                    ...
                    print('No known method specified')
                    break

        _ = np.array(list(generate_configurations(amount_nodes)))
        data = self.line_list.T

        def update(num):
            ax_rrt.plot((data[0, :, num].flatten()), (data[1, :, num].flatten()), (data[2, :, num].flatten()),
        ...
        plt.title(method + " Robot Manipulator Combinations")

        # Animate plot if specified
        if animate_plot:
            ani1 = animation.FuncAnimation(fig2, update, frames=data.shape[2], interval=20, blit=False,
                                           repeat=False)
            if save_animation:
    ...
    def plot_path_3d(self, data, path, animate_path=True):
        ...
        ax4.voxels(self.x_obs, self.y_obs, self.z_obs, self.data_obstacles, facecolors=[1, 0, 0, 0.1],
                   edgecolors='grey')
        ...
        for i in range(data.shape[2]):
            ax4.plot(data[0, :, i].flatten(), data[1, :, i].flatten(), data[2, :, i].flatten(), linewidth=1,
                     color='red')
